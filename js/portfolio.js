    ...
    data.map((e, i) => {
        ...
        for (let a = 0; a < descs.length; a++) {
            ...
            if (a === 1 && e.keyword) {
                ...
                let text = "사용툴 : ";
                text += e.tools.join(); // join을 쓰면 ,가 붙지만 한 단어로 인식
        ...
        const $createList = (href, text, extra = false) => {
...
$CateList.forEach((e, i) => {
    ...
    e.addEventListener("click", function() {
        ...
        $CateList.forEach((el, index) => {
            $CateList[index].classList.remove("on"); // 클릭 안한 버튼들
        });

        const $attr = e.getAttribute("data-type");

        $CateList[i].classList.add("on"); // 클릭한 버튼만 on
        ...
        .then (function(res) {
            ...
            if ($attr === "전체") {
                ...
                PortData = res.data.Portfolio.filter(item => item.type == $attr); // 값을 바로 반환하기 때문에 {} 중괄호 생략, filter 쓸 땐 {} 잘 안 씀, 이유는 계산하거나 그런거 없이 값을 바로 보내기 때문에
            }
